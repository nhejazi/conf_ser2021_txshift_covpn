...
\subtitle{\small ``Methods for the thorny challenges of real studies'',
SER 2020}
...
\institute{
  \begin{figure}[!htb]
    ...
    \begin{minipage}{.65\textwidth}
      ...
      Joint work with David Benkeser, Mark van der Laan, Holly Janes, Peter
      Gilbert
    \end{minipage}%
...
\begin{frame}[c]{One-step and targeted minimum loss estimators}

\begin{center}
\begin{itemize}
  \itemsep10pt
  \item One-step estimator relies on a bias correction:
    \begin{equation*}\label{os}
      \Psi_n^{+} = \frac{1}{n} \sum_{i = 1}^n \overline{Q}_n(d(A_i, W_i), W_i)
      + D^{\star}(P_0^X)(X_i).
      \end{equation*}
  \item TML estimator constructs $\overline{Q}_n^{\star}$ via a (logistic)
  tilting model
    \begin{equation*}\label{tmle}
      \Psi_n^{\star} \coloneqq \Psi(P_n^{\star}) = \frac{1}{n} \sum_{i = 1}^n
        \overline{Q}_n^{\star}(d(A_i, W_i), W_i).
      \end{equation*}
  \item In this case, the efficient influence function (EIF) is
    \begin{equation*}
      D^{\star}(P_0^X)(x) = H(a, w)({y - \overline{Q}(a, w)}) +
      \overline{Q}(d(a, w), w) - \Psi(P_0^X),
    \end{equation*}
    where the auxiliary (``clever'') covariate is
    \begin{equation*}
...
\begin{itemize}
  ...
  \item Rather, we merely require the post-intervention quantity be seen in the
    observed data for given $a_i \in \mathcal{A}$ and $l_i \in \mathcal{W}$.
\end{itemize}
\end{center}
...
\note{
  \begin{itemize}
    \item We recommend using nonparametric methods for the initial estimators,
      ...
      $\Psi_n$.
    \item Intuition for the submodel fluctuation? Nah.
  \end{itemize}
}
