...
saveGIF(
  {
    for (iter in seq_along(qbar_and_shift)) {
      ...
      p_a_shifted <- df_input %>%
        ...
        xlab("") +
        ylab("") +
        xlab("Standardized immune marker activity") +
        ggtitle(paste("Exposure profile:", "E[S] =", round(a_mean, 2))) +
        theme_bw() +
        theme(
      ...
      p_qbar_shifted <- df_input %>%
        ...
        ylab("") +
        ylim(0, 1) +
        ylab("Population risk of infection") +
        ggtitle(paste("Counterfactual mean risk:", "E[Y] =",
                      round(qbar_mean, 2))) +
        theme_bw() +
        theme(
          text = element_text(size = 22),
  ...
  movie.name = here("img", "shift_animation.gif"),
  interval = 1,
  ani.width = 1100,
  ani.height = 700
)
